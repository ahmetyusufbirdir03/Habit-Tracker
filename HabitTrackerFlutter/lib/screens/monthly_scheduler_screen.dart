...
import 'package:habit_tracker_mobile/widgets/Schedulers/MonthlyScheduler/monthly_scheduler_update_form.dart';
...
import '../widgets/Schedulers/MonthlyScheduler/monthly_scheduler_picker.dart';
import '../widgets/animated_card.dart';
import '../widgets/info_card.dart';
import 'group_inside_screen.dart';

class MonthlySchedulerScreen extends StatefulWidget {
  final HabitDto habit;
  final HabitGroupDto habitGroup;

  const MonthlySchedulerScreen({
    super.key,
    required this.habit,
  ...
  @override
  State<MonthlySchedulerScreen> createState() => _MonthlySchedulerScreenState();
}
final GlobalKey<AnimatedStreakCardState> streakCardKey = GlobalKey<AnimatedStreakCardState>();

class _MonthlySchedulerScreenState extends State<MonthlySchedulerScreen> {
  ...
  List<MonthlySchedulerDto> _habitSchedulers = [];

  HabitDto? _habit;
  bool _isEditingNote = false;
  late TextEditingController _noteController;
  String _habitName = "Monthly Habit";
  String? _notes;
  ...
  void initState() {
    super.initState();
    _habit = widget.habit;
    _noteController = TextEditingController(text: _notes ?? "");
  ...
  Future<void> getHabitSchedulers(String habitId) async {
    ...
    final response = await _schedulerService.getMonthlySchedulersAsync(widget.habit.id!);

    if (response.isSuccess && response.data != null) {
      setState(() {
        _habitSchedulers = response.data!.items;
        _habitSchedulers.sort((a, b) => a.dayOfMonth!.compareTo(b.dayOfMonth!));
        _isLoading = false;
      });
    } else {
      setState(() {
  ...
  Future<bool> completeScheduler(String schedulerId) async {
    ...
    var response = await _schedulerService.completeMonthlySchedulerAsync(schedulerId);
  ...
  Future<bool> updateScheduler({
    ...
    try {
      final response = await _schedulerService.updateMonthlySchedulerAsync(
          id: id,
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) async {
              if (value == 'update') {
                ...
                bool? isAccept = await ConfirmationPopup.show(context, "Are you sure you want to delete this habit?");
                if(isAccept == true){
        ...
        child: Column(
          children: [
              ...
              children: [
                Expanded(
                  child: Container(
                    ...
                    child: Center(
                      child: Text(
                        "Status : ${_habit!.isActive! ? "Active" : "Inactive"}",
                        style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                        ),
            ...
            AnimatedStreakCard(
              ...
              label: "Streak",
              value: "${_habit!.streak} months",
              subtitle: "Best Streak: ${_habit!.bestStreak} months",
            ),
            infoCard(
                ...
                children: [
                  ...
                  _isEditingNote
                    ...
                    child: TextField(
                      ...
                      : Text((_habit!.notes == null || _habit!.notes!.trim().isEmpty)
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          final bool? isSucceed= await showModalBottomSheet<bool>(
                            ...
                            builder: (context) => SimpleMonthlyPicker(habit: _habit!),
                          );
                          if(isSucceed!){
                            setState(() {
                              getHabitSchedulers(_habit!.id!);
                ...
                children: [
                  Center(
                    child: Container(
                      ...
                      child: const Text(
                        "*This schedule repeats on the same day each month. For example, "
                            "if you pick 3rd November, it will automatically appear on 3rd December.*",
                        style: TextStyle(
                          fontSize: 12,
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Builder(
                                    builder: (context) {
                                      final dateResult = formatDayMonth(day: scheduler.dayOfMonth!);
                                      final isError = dateResult.error != null;
                                      var firstText = "";
                                      if(!isError){
                                        firstText = "The reminder's date is:\n";
                                      }
                                      return Text(
                                        "$firstText"
                                        "${dateResult.toString()}"
                                ...
                                IconButton(
                                  ...
                                  onPressed: () async {
                                    final result = await showModalBottomSheet(
                                      ...
                                      isScrollControlled: true,
                                      backgroundColor: Colors.transparent,
                                      builder: (context) => MonthlySchedulerEditPicker(scheduler: scheduler),
                                    );
                                    if (result != null) {
